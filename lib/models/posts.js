...
function allPublished(page, cb) {
  var q, byPage = settings.get('POSTS_BY_PAGE');
  ...
  countPublished(function(err, count) {
    ...
    q += ' LIMIT ' + byPage + ' OFFSET ' + (page - 1) * byPage;

    query(q, handleAll(count, cb));
  });
...
function all(page, cb) {
  var q, byPage = settings.get('POSTS_BY_PAGE_ADMIN');
