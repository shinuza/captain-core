 ...
 *
 * `cb` is passed with the matching post, or exceptions.NotFound
 *
 * @param {String} slug
 * @param {Function} cb
 */
 ...
 *
 * `cb` is passed with the updated post, or exceptions.NotFound
 *
 * @param {Number} id
 * @param {Object} body
 * @param {Function} cb
...
function allPublished(page, cb) {
  var q,
    limit = Number(conf.posts_by_page),
    offset = (page - 1) * limit;
...
function all(page, cb) {
  var q,
    limit = Number(conf.posts_by_page_admin),
    offset = (page - 1) * limit;
