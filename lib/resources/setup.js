...
function database(req, res, next) {
  var body = req.body,
      str = settings.TMPL({key: 'DB', value: '"' + body.uri + '"'});

  fs.writeFile(settings.path(settings.DEFAULT_FILENAME), str,  function(err) {
    if(err) return next(err);
...
function connectionTest(req, res, next) {
  var s = require(settings.path(settings.DEFAULT_FILENAME));
